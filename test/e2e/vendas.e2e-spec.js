...
describe('Vendas E2E', () => {
  ...
  it('deve registrar uma venda com dados válidos e enviar email à editora', async () => {
    // arrange
  ...
  it('deve retornar 400 ao registrar venda com valor não numérico', async () => {
    ...
    const resposta = await request(app).post('/vendas').send({ idLivro: livro.id, valor: 'abc', modoPagamento: 'PIX' });
  ...
  it('deve retornar 404 ao registrar venda para um livro inexistente', async () => {
    ...
    const resposta = await request(app).post('/vendas').send({ idLivro: 999999, valor: 100, modoPagamento: 'PIX' });
  ...
  it('deve listar as vendas registradas', async () => {
    const livro = await criarLivro(db);
    const app = criarAppDeTeste();
    await request(app).post('/vendas').send({ idLivro: livro.id, valor: 100, modoPagamento: 'PIX' });
