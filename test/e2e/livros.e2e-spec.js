...
describe('Livros E2E', () => {
  ...
  describe('casos de sucesso', () => {
    ...
    it('deve buscar livros por trecho do título e faixa de páginas', async () => {
      const autor = await criarAutor(db);
      const editora = await criarEditora(db);
      await request(app).post('/livros').send({ titulo: 'Dom Casmurro', paginas: 256, autor_id: autor.id, editora_id: editora.id });
      await request(app).post('/livros').send({ titulo: 'Dom Quixote', paginas: 900, autor_id: autor.id, editora_id: editora.id });
