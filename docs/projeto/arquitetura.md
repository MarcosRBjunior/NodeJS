...
- Login de cliente com JWT já existe (2026-09-07, ver `docs/prd/login-cliente/`) — checkout exige conta, e `/admin` exige papel admin. Estoque real também já existe (2026-09-08, ver `docs/prd/estoque-real/`, ADR-008). Pagamento real segue como lacuna conhecida e aceita por enquanto — candidata a entrar como feature formal (com `requirements.md` + `test-plan.md` próprios) quando priorizada.
...
- **Injeção de dependência é seletiva.** `emailGateway`/`stockGateway` percorrem `server → app → routes → service` como parâmetros (por isso são fáceis de mockar em teste). A conexão com o banco é um singleton (`obterConexao()`) importado direto em cada arquivo de rota — o banco é sempre real, inclusive em testes de integração/e2e. Desde a feature de estoque real (2026-09-08, ADR-008), `stockGateway` deixou de ser um side effect externo mockável — ele opera na mesma tabela `livros` do banco real, então só `emailGateway` é mockado por padrão nos testes.
...
### ADR-008 — Decremento de estoque como UPDATE atômico dentro da mesma transação da venda, sem lock explícito

- **Contexto**: a feature de estoque real (2026-09-08, ver `docs/prd/estoque-real/`) precisa impedir que duas compras simultâneas vendam mais unidades do que existem em `livros.estoque_quantidade`. `StockGateway` deixou de ser um mock fixo (`consultarEstoque` sempre `true`) e passou a fazer `decrementarEstoque` de verdade.
- **Decisão**: o decremento é um único `UPDATE livros SET estoque_quantidade = estoque_quantidade - :quantidade WHERE id = :livroId AND estoque_quantidade >= :quantidade` (via `knex .decrement()` com `.andWhere`), checando `linhasAfetadas > 0`, executado dentro da mesma transação de banco que insere a `venda` (`VendasService.registrarVenda` usa `db.transaction(...)`; `RepositorioBase.salvar` ganhou um parâmetro opcional de conexão pra poder inserir usando a `trx`). Nenhum `SELECT ... FOR UPDATE` ou lock explícito foi adicionado.
- **Motivo**: um `UPDATE` com `WHERE` sobre uma condição do próprio valor da linha já é atômico no Postgres — a segunda transação concorrente bloqueia até a primeira commitar, e então reavalia o `WHERE` contra o valor já commitado (READ COMMITTED, o isolation level default). Isso resolve a condição de corrida sem precisar de `SELECT FOR UPDATE`, `SERIALIZABLE` ou lock de aplicação — menos código, mesmo resultado. Validado com um teste de integração que dispara duas compras concorrentes contra um livro com `estoque_quantidade = 1` (`test/integration/vendas-service.integration-spec.js`): exatamente uma sucede, a outra recebe `Conflito` (409), estoque final fica em `0` (nunca negativo).
- **Trade-offs reconhecidos**: essa garantia depende de o `WHERE` do `UPDATE` incluir a condição de suficiência (`estoque_quantidade >= quantidade`) — um `UPDATE` sem essa cláusula (só por `id`) não teria a mesma segurança. Também depende de todo decremento de estoque passar por esse caminho único (`StockGateway.decrementarEstoque`); um `UPDATE` direto na tabela fora desse gateway quebraria a garantia.
- **Status**: aceito.
